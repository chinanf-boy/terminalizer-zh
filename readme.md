...
> 如果你这么认为,请给我一个支持`star`和a`follow`😘
...
- [配置](#%E9%85%8D%E7%BD%AE-1)
  ...
  - [Frame 框](#frame-%E6%A1%86)
    ...
    - [style提示](#style%E6%8F%90%E7%A4%BA)
- [常问问题](#%E5%B8%B8%E9%97%AE%E9%97%AE%E9%A2%98)
    - [如何支持ZSH](#%E5%A6%82%E4%BD%95%E6%94%AF%E6%8C%81zsh)
...
-   配置能力: 
    -   能捕获的命令终端 (bash,powershell.exe,yourOwnCommand等) 
    -   当前的工作目录. 
    -   列和行数的显式值. 
    -   GIF质量和重复. 
    -   帧延迟. 
...
GIF压缩尚未实施. 现在我们推荐<https://gifcompressor.com>. 
...
-   `env`: 导出其他ENV变量,在开始录制时由脚本读取. 
-   `cols`: 显式设置列数或使用`auto`获取shell的当前列数. 
-   `rows`: 显式设置行数或使用`auto`获取shell的当前行数. 

## Delays

-   `frameDelay`: 以ms为单位的帧之间的延迟. 如果值是`auto`使用实际录制延迟. 
...
-   `fontFamily`: 您可以使用计算机上安装的任何字体`Monaco`要么`Lucida Console` (类似CSS的列表) . 
...
-   `watermark.imagePath`: 计算机上图像的绝对路径或URL. 
-   `watermark.style`: 将CSS样式 (camelCase) 应用于水印图像,如调整大小. 

## Frame 框

终结器附带预定义的框架,您可以使用它们使您的GIF图像看起来很酷. 
...
为Linux录制的默认命令是`bash -l`. 您需要将默认命令更改为`zsh`. 
